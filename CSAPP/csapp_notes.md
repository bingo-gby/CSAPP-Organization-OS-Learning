...
- [6. 异常控制流（ECF）](#6-异常控制流ecf)
  - [1. 进程](#1-进程)
    - [1. 子进程跟父进程的关系](#1-子进程跟父进程的关系)
  - [2. 信号](#2-信号)
    - [1. 发送信号](#1-发送信号)
    - [2. 接收信号](#2-接收信号)
    - [3. 阻塞和接触阻塞信号](#3-阻塞和接触阻塞信号)
    - [4. 信号处理程序的编写](#4-信号处理程序的编写)
- [7. 系统级I/O](#7-系统级io)
- [8. 虚拟内存](#8-虚拟内存)
...
# 6. 异常控制流（ECF）
异常控制流（Exceptional Control Flow）是计算机程序中的一种控制流，它偏离了程序正常的执行顺序，通常是由于发生了某些异常情况。在异常控制流中，程序的执行路径会因为特定的事件（**如错误、系统调用、信号、上下文切换等**）而改变，转向异常处理代码。  
异常是将控制权转移到内核。异常存在一个异常表。  
<img src="photos/int.png" width="75%"> 

通常异常处理程序完成处理后，会发生以下情形之一：
1. 将控制返还给当前指令
2. 将控制返还给下一条指令（没有发生异常将会执行的下一条指令）
3. 处理程序终止被中断的程序

异常分为异步异常和同步异常。异步异常， 如中断（由处理器外部的IO设备的信号的结果）。同步中断分为 系统调用、故障和终止。主要就是系统调用，常见的系统调用如下：  
<img src="photos/sys.png" width="75%">   

以open为例：open调用 sysopen函数，如下所示：    
<img src="photos/open.png" width="75%">   

汇编语句解释如下：
1. 先把系统调用号存入 eax寄存器，然后调用 sysopen
2. 返回值存到 rax寄存器，如果是负值，就是调用错了，返回值是一个文件描述符的整数。
   
故障举个例子：如页错误，发现页面不在物理内存中，需要从磁盘加载。

## 1. 进程
进程就是运行程序的实例，进程提供两个抽象：（1）进程独立占用CPU和寄存器（2）有独立的地址空间，由虚拟内存机制提供。进程的时间流上存在重叠，就称为并发。  
进程控制中，通常要检查每个系统调用函数的返回值，如果是-1，那就是出现错误了，需要设置 全局变量errno来指示错误原因。示例。用包装器来指示错误。
```
...
// 这样直接调用就行，用户也可以不去检查返回值
pid = Fork()
```
另外 getpid() 函数是返回当前进程pid号，getppid()是返回父进程pid号。一般进程存在三个状态：运行 挂起 终止（五态模型是 新建 就绪 运行 堵塞 终止），通常的终止方法有三种：（1）接收到终止信号（2）正常main函数返回（3）调用exit函数。exit函数参数为status，如果是0就是正常返回，反之就是不正常返回。创建进程用fork()函数，它奇怪的一点是调用一次，返回两次，对于父进程，返回子进程的pid，对于子进程返回0,子进程获得父进程的虚拟地址空间的副本，之后运行的是同一个程序，但是无法预测子父进程之间的运行顺序。可以尝试用进程图来看看子父进程之间的语句执行顺序。   
<img src="photos/fork.png" width="75%">  
可以看到子进程返回2，父进程返回0，是因为最开始获得的x都是1，但是之后两个进程对其进行的转换是独立的。 进程图如下所示：  
<img src="photos/fork1.png" width="75%">  
可以看到创建了4个进程（调用2次fork）。

### 1. 子进程跟父进程的关系
子进程终止时，内核不会直接将它从系统中清除，而是保持一种终止状态，等待父进程将其回收（wait or waitpid）。终止但未回收就称之为僵尸进程（半死不活）。如果父进程终止了，就会安排init进程（PID=1）为孤儿进程的养父，或者安排init进程回收僵尸进程。僵尸进程会占用内核资源，所以需要去回收。  
wait和 waitpid函数解释：  
>int wait(int* child_status) 这是wait函数的参数和返回值，返回值是终止的子进程的pid，调用wait函数之后，父进程就被挂起了，等待子进程来唤醒.  
>waitpid函数，更复杂点，是等特定进程终止。`pid_t waitpid(pid_t pid, int &status, int options)`

上面用fork的进程跟父进程运行的是一样的代码，如果想运行不同的代码，用 execve函数，在进程的上下文加载并运行一个新程序，值得注意的是该函数会覆盖当前进程的地址空间，新的程序仍然拥有相同的pid并且继承了调用execve时已打开的所有文件描述符（就是文件使用权限都保留了）。execve函数使用示例：  
<img src="photos/execve.png" width="75%">  
就是希望在当前程序中调用ls函数，之后主程序还能正常工作，就可以遵循以上做法（fork和execve合作使用），但是这个存在问题，就是没有回收子进程，在后面的信号这一讲将会介绍。

## 2. 信号
linux中，init进程先会创建守护进程（你希望一直运行在后台中的进程）和登录进程（登录shell），然后通过shell命令去创建其他 Child 或者 GrandChild。linux默认的shell是bash。  
Signal就是一条小消息，它通知内核发生了某种类型的事件。信号的唯一信息就是ID和其标志的相应时间（具体见课本527页）。（Shell中就使用了信号，子进程结束后通知父进程回收，要不然就成了僵尸进程了）。  
信号分为发送信号和接受信号。发送信号通常有两种原因：（1）内核检测到系统事件，例如除零错误或是子进程终止（2）某进程调用了kill函数。进程接受信号可以有以下几种行为：（1）忽略信号（2）终止进程（3）用一个信号处理程序的用户级函数来捕获信号。
1. 一个发出而没有被接收的信号称为待处理信号。任何时刻，一个类型只能有一个待处理信号（就是假如有个信号是待处理，那后续发的该信号都会直接被丢弃）。
2. 进程也可以选择对信号的堵塞（就是不接受该信号），直到进程取消堵塞，才可以接受。
3. 一个待处理信号最多被接收一次，接收完了就没了。
4. 内核为每个进程维护着待处理信号集合（在pending位向量中）和被堵塞的信号集合（在blocking位向量中），通过设置和清除位向量中的位来实现

### 1. 发送信号
首先理解进程组的概念，进程组由ID标识，一般父子进程都是同一进程组。getpgrp函数返回当前进程组的ID，也可以通过`int setpgid(pid_t pid, pid_t pgid)` 将进程pid的进程组改为pgid，成功返回0，错误返回-1。进程组的概念允许同时给一组进程发送信号。

1. /bin/kill 发送kill信号  
   /bin/kill -9 15123 发送SIGKILL（9）信号给进程15123  
   /bin/kill -9 -15123 发送SIGKILL（9）信号给进程组15123的每个进程
2. 从键盘发送信号：shell使用 作业 概念，一个命令行就是一个作业，一般只会有一个前台作业和多个后台作业。例如 `ls | sort` 用于列出当前目录下的文件和目录，并将它们排序。这个命令会创建两个进程组成的前台作业，每个作业是一个独立的进程组。CTRL+C会发送 SIGINT 信号到前台进程组的每个进程，通常是终止前台作业，类似的 输入 CTRL+Z会挂起前台作业(发送 SIGTSTP信号，收到SIGCONT信号才重启)。
3. kill函数发送信号： `int kill(pid_t pid,int sig)`，if pid >0,那么就发送sig给进程pid；if pid==0，发送sig给调用进程所在进程组的每个进程；if pid<0, 发送sig给 进程组|pid|的每个进程

### 2. 接收信号
内核将进程从内核模式切换到用户模式时，会检查进程未被阻塞的待处理信号的结合，如果没有，直接执行进程p的下一条指令。如果有，内核选择最小的信号强制进程接收信号，触发进程采取某种行为。每个信号类型都有默认行为，如终止、挂起或是忽略。如果想修改其默认行为，可以通过 `signal` 函数。
1. sighandler_t signal(int signum,sighandler_t handler) 为函数
2. handler 为 SIG_IGN 就是设置忽略
3. handler 为 SIG_DFL 就是恢复默认行为
4. handler 为自定义函数地址，就是信号处理程序，称为捕获信号，另外信号处理程序同样可以被信号处理程序中断。

信号也是并发的...只要时间流上存在重叠，就是并发...

### 3. 阻塞和接触阻塞信号
隐式阻塞机制：假如程序捕获信号s，正在处理，这个时候重新发送一个s给进程，那么直到处理程序返回，s变为待处理信号，不会接受。  
显示阻塞机制：通过sigprocmask 函数 和一些辅助函数。
> `int sigprocmask(int how,const sigset_t *set,sigset_t *oldset)`  用来改变当前阻塞的信号集合，依赖于how的值： SIGBLOCK：将set中加入blocked；SIG_UNBLOCK：从blocked删除set中的信号； SIG_SETMASK： blocked = set， blocked的值存在oldset中。辅助函数对set操作如下。  
...
sigemptyset(&mask) // 初始化
sigaddset(&mask,SIGINT) // 将SIGINT信号加进去

sigprocmask(SIG_BLOCK,&mask,&prev_mask)  // prev_mask 存储之前的blocked

sigprocmask(SIG_SETMASK,&prev_mask,NULL) // 将blocked设为之前的 prev_mask

// 主要就是实现，先暂时阻塞 SIGINT，然后取消阻塞...
```
### 4. 信号处理程序的编写
信号处理存在几个问题：（1）与主程序并发且共享同样的全局变量，可能会产生相互干扰。这里提供一些指导方案。  
1. 尽可能使处理程序简单点
2. 处理程序中只调用异步信号安全的函数
3. 进入处理程序时，将errno存在一个局部变量中，处理程序返回后恢复它，因为这是一个全局变量
4. 通过临时阻塞所有信号来保护对共享数据结构的访问
5. 将全局变量声明为volatile，告诉编译器不缓存该变量，每次都要重新从内存中读取
6. 全局标志声明为 volatile sig_atomic_t ，保证对其读写操作是原子的，不可中断的

上面提到异步安全，异步安全的函数能被信号处理程序安全的调用：原因在于：（1）可重入的（例如只访问局部变量，具体后续并发编程可以自己去看）（2）不能被信号处理程序中断。printf不是异步安全的，举例main函数调用了 printf，然后信号处理函数接管，调用printf，但是此时终端被printf使用，所以信号处理函数就堵塞，造成死锁的情况..唯一安全的是使用 write函数。  
同时未处理的信号是不排队的会导致问题。基本结构是父进程创建子进程，但是希望父进程能够在子进程运行期间自由地去做其他的工作，所以决定使用 SIGCHID 来回收子进程（只要有一个子进程终止，内核发送 SIGCHID 信号给父进程），所以通常不可以用信号来对其他进程中发生的事件计数...(书上貌似选择while循环来解决) 。  
信号的移植之类的自己看书吧...这里不赘述。

# 7. 系统级I/O
暂时跳过... 有需求自己看书
# 8. 虚拟内存
虚拟内存非常重要，主存是一个字节数组，每字节存在唯一的物理地址（Physical address）。以前可能使用的居多，但是后面都采用 虚拟地址（virtual address）-> MMU（memory management unit,内存管理单元）-> PA。地址空间指就是 虚拟地址 和 物理地址。一个包含 N = 2^n 个地址的虚拟地址空间就叫做一个 n位地址空间，例如32位地址空间，64位地址空间等。物理地址空间同样，主存内的每个字节都有一个虚拟地址和物理地址。  
