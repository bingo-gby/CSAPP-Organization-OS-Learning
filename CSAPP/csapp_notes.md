...
使用block（分块）改变时间局部性。就是矩阵分块乘法，一次取的多了，之后就更容易缓存命中。举例说明，假设是按 ijk展开，缓存块=8 doubles，缓存大小C << n。之前的就是 9/8 n^3 命中次数。如果使用分块矩阵，假设原矩阵 32 * 32，分成 4个 16*16。矩阵乘法就是：A矩阵的第一小块第一行和B矩阵的第一小块的第一列进行运算。然后加上A矩阵的第二小块的第一行和B矩阵第三小块的第一列进行运算。得到的结果就为C[0][0]，依次类推。
