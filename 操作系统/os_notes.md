...
#### 1. 生磁盘到文件
前面讲的是如何从 盘块号 到 具体磁盘上，但实际使用都是从文件得到盘块号，或者说在盘块上引入更高层次的抽象概念：文件。文件就是建立 字符流到 盘块集合的映射...实现文件可以有多种结构实现这种映射关系.. 这种映射关系存在 FCB里面，文件的FCB..举例说明，FCB是什么，假如是连续结构，test.c对应盘块 7 8 9，那么test.c 的FCB如下：  
<img src="photos/FCB.png" width="75%">   
...
>目录的使用：实际通过 /my/data/a 定位文件a，找到文件的FCB和Inode,是怎么实现的？（FCB包含文件的一些信息） 答：假如/ 下有两个目录，var 和 my,数据块放对应的索引，然后去FCB数组找..   
    <img src="photos/cipan1.png" width="75%">  

同时为了让系统能够自举，还需要存一些其他的信息 
